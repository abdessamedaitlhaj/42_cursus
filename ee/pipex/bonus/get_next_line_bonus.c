...
/*   Updated: 2024/06/05 16:14:55 by aait-lha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*right_str(char *st_var)
{
	...
	if (!right_str)
		(free(st_var), perror("malloc error"), exit(1));
	i++;
	j = 0;
	while (st_var[i])
	{
...
static char	*read_line(int fd, char *st_var)
{
	...
	while (nb_bytes != 0 && !ft_strchr(st_var, '\n'))
	{
		...
		if (!st_var)
			(free(buffer), perror("malloc error"), exit(1));
		if (nb_bytes == 0 && st_var[0])
			nb_bytes = 1;
	}
	free(buffer);
	return (st_var);
